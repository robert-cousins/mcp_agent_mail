...
from mcp_agent_mail.macros.send_ready_report import AMClientProtocol, send_ready_report


@pytest.fixture
def mock_am_client() -> MagicMock:
    client = MagicMock()
    client.send_message.return_value = {"deliveries": [{"id": 99}]}
    return client

@patch("mcp_agent_mail.macros.send_ready_report.ci_gate")
def test_send_ready_report_gate_fails(mock_ci_gate: MagicMock, mock_am_client: MagicMock) -> None:
    # Setup ci_gate to fail
    mock_ci_gate.return_value = {
        "ok": False,
        "sha": "sha123",
        "failed": [{"name": "Check", "reason": "failure"}],
    }

    am_client = cast(AMClientProtocol, mock_am_client)
...
@patch("mcp_agent_mail.macros.send_ready_report.ci_gate")
def test_send_ready_report_gate_passes(mock_ci_gate: MagicMock, mock_am_client: MagicMock) -> None:
    ...
    call_args: dict[str, Any] = mock_am_client.send_message.call_args[1]
    body = cast(str, call_args["body_md"])
